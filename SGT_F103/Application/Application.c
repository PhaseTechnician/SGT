...
#define CreatNewPipe(handler, length, messageStruct, pipeName)        \
	{                                                                 \
		handler = xQueueCreate(length, sizeof(messageStruct));        \
		if (handler == NULL)                                          \
		{                                                             \
			BSP_Serial_SendString("Application_Pipe_Creat_Failed\n"); \
			BSP_Serial_SendString(pipeName);                          \
			BSP_Serial_SendChar('\n');                                \
		}                                                             \
	}
